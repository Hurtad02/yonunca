package com.example.yonunca_juegoparabeber.naughty.viewmodel
...
class OfflineScreenViewModel: ViewModel() {
    ...
    private val defaultPhrase = Phrase(
        ...
        likes = 0,
        collection = NAUGHTY_COLLECTION
    )

    private val uiState: MutableLiveData<OfflineUIState> =
        MutableLiveData<OfflineUIState>().also {
            it.value = OfflineUIState(defaultPhrase, shouldEnableSpeech = false)
        }
    ...
    fun getPhrases(){
        viewModelScope.launch(Dispatchers.IO) {
            val fetchedPhrases = model.getNaughtyPhrases()
